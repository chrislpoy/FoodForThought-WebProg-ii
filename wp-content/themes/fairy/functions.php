...
function fairy_scripts()
{
	...
	$fairy_fontawesome = !empty($fairy_theme_options['fairy-font-awesome-version-loading']) ? esc_attr($fairy_theme_options['fairy-font-awesome-version-loading']) : 'version-4';

	if ($fairy_fontawesome == 'version-5') {
		/*Font-Awesome-master*/
	...
	if (!empty($fairy_theme_options['fairy-enable-sticky-sidebar']) && 1 == absint($fairy_theme_options['fairy-enable-sticky-sidebar'])) {
	...
	if (!empty($fairy_theme_options['fairy-pagination-options'])) {
		$fairy_pagination_option = $fairy_theme_options['fairy-pagination-options'];
		if ($fairy_pagination_option == 'ajax') {
			wp_enqueue_script('fairy-custom-pagination', get_template_directory_uri() . '/candidthemes/assets/custom/js/custom-infinte-pagination.js', array('jquery'), _S_VERSION, true);
			global $wp_query;
			$paged = (get_query_var('paged') > 1) ? get_query_var('paged') : 1;
			$max_num_pages = $wp_query->max_num_pages;

			wp_localize_script('fairy-custom-pagination', 'fairy_ajax', array(
				'ajaxurl' => esc_url(admin_url('admin-ajax.php')),
				'paged' => absint($paged),
				'max_num_pages' => absint($max_num_pages),
				'next_posts' => next_posts(absint($max_num_pages), false),
				'show_more' => __('Load More', 'fairy'),
				'no_more_posts' => __('No More', 'fairy'),
...
function university_post_types(){
        ...
        register_post_type('Medicals',array(
            'supports' => array('title', 'editor'),
            'rewrite'=> array('slug' => 'medicals' ),
            'has_archive' => true,
            'public' => true, 
            'labels' => array(
                'name' => "Medicals",
                'add_new_item' => 'Add New Medical',
                'edit_item' => 'Edit Medical',
                'all_items' => 'All Medicals',
